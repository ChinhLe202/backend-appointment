    ...
    $("#createNewClinic").on('click', function(e) {
        ...
        if ($('#image-clinic').val()) {
            ...
            reader.onloadend = function() {
                // `reader.result` sẽ chứa chuỗi Base64 của ảnh
                let base64String = reader.result.split(',')[1]; // Bỏ phần tiền tố "data:image/jpeg;base64,"
                ...
                console.log(data);
                // Sau khi chuyển đổi Base64, tiếp tục gửi request cập nhật
                handleCreateClinicNormal(data);
            };

            // Đọc file ảnh dưới dạng chuỗi Base64
...
function updateDoctor() {
    $('#btnUpdateDoctor').on('click', function(e) {
        ...
        if ($('#image-clinic').val()) {
            ...
            // Nếu không có ảnh, chỉ gửi các dữ liệu khác
            let data = {
                id: doctorId,
            };
